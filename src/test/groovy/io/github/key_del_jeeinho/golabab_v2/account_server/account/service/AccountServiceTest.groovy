...
class AccountServiceTest extends Specification {
    ...
    - 또한, id, email, discordId 는 중복되지 않아야 한다.
     */
    def "AccountService 의 addAccount 메서드에 대한 Positive Test"() {
        ...
        accountRepository.save(new AccountEntity(계정id, 이메일, 역할, 디스코드id)) >> new AccountEntity(결과값_계정id, 이메일, 역할, 디스코드id)
        ...
        then:
        println (accountRepository.save(new AccountEntity(계정.id(), 계정.email(), 계정.role(), 계정.discordId())))
        결과값.id() == 결과값_계정id
        결과값.email() == 계정.email()
        결과값.role() == 계정.role()
